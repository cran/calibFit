...
%\VignetteIndexEntry{calib}
%\VignetteKeywords{calib}
...
the calib package and its application to the latter type of problem.
\end{abstract}
...
The package we have developed,  \Rpackage{calib}, performs univariate calibration
...
curve. The calib package is designed to be useful for both statisticians and 
...
detailed discussion of the model fitting in the \Rpackage{calib} package.
...
The \Rpackage{calib} package implements two methods for model fitting, 
...
\noindent First load the \Rpackage{calib} library
...
\noindent In the paper by Davidian and Haaland and currently in the \Rpackage{calib} package
...
\Rpackage{calib} has two options for calculating the confidence intervals for the calibrated
...
than a fixed percent, the default is 20 in the \Rpackage{calib} package.
